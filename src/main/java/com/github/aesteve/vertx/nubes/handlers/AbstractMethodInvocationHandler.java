...
public abstract class AbstractMethodInvocationHandler<T> implements Handler<RoutingContext> {
	...
	protected AbstractMethodInvocationHandler(Object instance, Method method, Config config,
			boolean hasNext, BiConsumer<RoutingContext, T> returnHandler)
	{
		this.method = method;
	...
	protected Object[] getParameters(RoutingContext routingContext) throws WrongParameterException
	{
		...
		for (Parameter param : parameters)
		{
			...
			Object paramInstance = getParameterInstance(
					routingContext,
					param.getAnnotations(),
					param.getType(), paramName);
			...
			Class<?> parameterClass, String paramName) throws WrongParameterException
	{
		...
		final AnnotatedParamInjector injector = annotatedInjectors
				.getInjector(annotation.annotationType());
		if (injector == null)
		{
			return null;
		}
